(()=>{
  ...
  async function renderAccess(owner){
    ...
    try{
      ...
      await publicIdentity;el.textContent='VIEW ONLY · SIGN IN FOR MANAGEMENT';el.dataset.access='view';publishEconomy(owner,{equipped:PUBLIC_EQUIPPED},false,'view');
      const btn=q('#openCrateBtn');if(btn){btn.disabled=true;btn.textContent='SIGN IN TO OPEN'}
    }catch(error){await publicIdentity;if(Number(ACTIVE_OWNER?.roster?.roster_id)!==rosterId)return;console.error('HQ access/economy check failed',error);if(!ECONOMY)publishEconomy(owner,{equipped:PUBLIC_EQUIPPED},false,'error');el.textContent='VIEW ONLY · ACCESS CHECK UNAVAILABLE';el.dataset.access='error';const btn=q('#openCrateBtn');if(btn){btn.disabled=true;btn.textContent='HQ ECONOMY UNAVAILABLE'}}
  }
