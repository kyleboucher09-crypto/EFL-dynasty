...
import { auth, queueEflEmail } from './efl-auth.js';
...
let eflSchemaReady = globalThis.__eflAccountSchemaReady || null;
export async function ensureEflAccountSchema() {
  if (!eflSchemaReady) {
    eflSchemaReady = (async () => {
      const sql = sqlClient();
      await sql`
    ...
    globalThis.__eflAccountSchemaReady = eflSchemaReady;
  }
  return eflSchemaReady;
}
...
export async function ensureProfile(user) {
  await ensureEflAccountSchema();
  const sql = sqlClient();
  const commissionerEmail = String(process.env.COMMISSIONER_EMAIL || '').trim().toLowerCase();
  const email = String(user.email || '').trim().toLowerCase();
  const role = commissionerEmail && email === commissionerEmail ? 'commissioner' : 'member';
  const displayName = String(user.name || email.split('@')[0] || 'EFL Member').trim().slice(0, 120);
  await sql`
...
export async function accountSnapshot(user) {
  ...
  const [profiles, memberships, claims, ownerships] = await Promise.all([
    ...
    sql`SELECT league_id, roster_id, approved_at FROM efl_franchise_owners WHERE user_id=${user.id} ORDER BY approved_at DESC`,
  ]);
...
async function sleeperFranchise(league, rosterId) {
  ...
  const franchiseName = sleeperUser?.metadata?.team_name || sleeperUser?.display_name || `Roster ${roster.roster_id}`;
...
export async function submitFranchiseClaim(user, leagueId, rosterId) {
  ...
  const validated = await sleeperFranchise(league, rosterId);
  const sql = sqlClient();

  const owned = await sql`SELECT user_id FROM efl_franchise_owners WHERE league_id=${league.id} AND roster_id=${Number(rosterId)} LIMIT 1`;
  ...
  try {
    const rows = await sql`
      ...
      VALUES (${user.id}, ${String(user.email).toLowerCase()}, ${String(user.name || user.email)}, ${league.id}, ${league.name}, ${Number(rosterId)}, ${validated.franchiseName})
      RETURNING id, league_id, league_name, roster_id, franchise_name, status, requested_at
    `;
    claim = rows[0];
  } catch (error) {
    if (String(error?.message || '').includes('uq_efl_pending_claim_user_league')) throw Object.assign(new Error('You already have a pending franchise claim in this league.'), { status: 409 });
    if (String(error?.message || '').includes('uq_efl_pending_claim_franchise')) throw Object.assign(new Error('Another manager already has a pending claim for that franchise.'), { status: 409 });
    throw error;
  }

  await sql`INSERT INTO efl_account_audit_log(actor_user_id, action, league_id, roster_id, target_user_id, detail) VALUES (${user.id}, 'franchise_claim_requested', ${league.id}, ${Number(rosterId)}, ${user.id}, ${validated.franchiseName})`;

  const commissionerEmail = String(process.env.COMMISSIONER_EMAIL || '').trim();
  if (commissionerEmail) {
    const reviewUrl = `${process.env.BETTER_AUTH_URL || 'https://www.efldynasty.com'}/commissioner-dashboard.html#account-claims`;
    queueEflEmail({
      ...
      html: `<div style="font-family:Arial,sans-serif;background:#07101d;color:#f4f8ff;padding:26px"><div style="max-width:580px;margin:auto;background:#0c1a2d;border:1px solid #234a7a;border-radius:18px;padding:24px"><div style="color:#ffd979;font-weight:900">NEW FRANCHISE CLAIM</div><h2>${validated.franchiseName}</h2><p><b>League:</b> ${league.name}<br><b>Manager:</b> ${user.name}<br><b>Email:</b> ${user.email}</p><p><a href="${reviewUrl}" style="display:inline-block;padding:12px 18px;border-radius:10px;background:#1769e8;color:white;text-decoration:none;font-weight:800">REVIEW CLAIM</a></p><p style="color:#8092aa;font-size:12px">Approval requires a secure commissioner session on EFLDynasty.com.</p></div></div>`,
    });
  }
  return claim;
}
...
export async function reviewFranchiseClaim({ claimId, status, reviewerUserId, reviewNote }) {
  ...
  if (status === 'approved') {
    ...
    await sql`UPDATE efl_franchise_claims SET status='denied', reviewed_at=NOW(), reviewed_by=${reviewerUserId || 'commissioner'}, review_note='Another claim for this franchise was approved.' WHERE league_id=${claim.league_id} AND roster_id=${claim.roster_id} AND status='pending' AND id<>${claim.id}`;
  }

  const updated = await sql`
    UPDATE efl_franchise_claims
    SET status=${status}, reviewed_at=NOW(), reviewed_by=${reviewerUserId || 'commissioner'}, review_note=${String(reviewNote || '').slice(0, 500)}
    WHERE id=${claim.id}
  ...
  const accountUrl = `${process.env.BETTER_AUTH_URL || 'https://www.efldynasty.com'}/account.html`;
  queueEflEmail({
    to: claim.user_email,
