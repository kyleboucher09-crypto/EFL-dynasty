...
export async function submitFranchiseClaim(user, leagueId, rosterId) {
  ...
  const existingOwnership = await sql`SELECT roster_id FROM efl_franchise_owners WHERE league_id=${league.id} AND user_id=${user.id} LIMIT 1`;
  if (existingOwnership.length) throw Object.assign(new Error('Your account already manages a franchise in this league.'), { status: 409 });
