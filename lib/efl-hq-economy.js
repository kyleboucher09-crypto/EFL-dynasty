...
export async function grantHqTestCrate({leagueId,rosterId,userId,note}){
  await ensureEflAccountSchema();
  ...
  return transaction(async client=>{
    ...
    const id=randomUUID();
    await client.query("INSERT INTO efl_test_crates(id,league_id,roster_id,status,granted_by_user_id,grant_note) VALUES($1,$2,$3,'ready',$4,$5)",[id,p.league.id,p.rosterId,userId,reason]);
    await client.query('INSERT INTO efl_account_audit_log(actor_user_id,action,league_id,roster_id,detail) VALUES($1,$2,$3,$4,$5)',[userId,'hq_commissioner_test_crate_grant',p.league.id,p.rosterId,JSON.stringify({testCrateId:id,note:reason})]);
    return {reward:{type:'commissioner_test_crate',note:reason},economy:await snapshot(client,p)};
  });
}
...
export async function mutateHqEconomy({leagueId,rosterId,userId,action,itemId,slot}){
  ...
  return transaction(async client=>{await lock(client,p.league.id,p.rosterId);let reward=null;
    if(operation==='purchase'){
      ...
      const opened=await client.query('SELECT win_key FROM efl_victory_crate_openings WHERE league_id=$1 AND roster_id=$2',[p.league.id,p.rosterId]),keys=new Set(opened.rows.map(row=>row.win_key));let winKey=p.victoryKeys.find(key=>!keys.has(key)),testCrate=null;
      if(!winKey){const test=await client.query("SELECT id FROM efl_test_crates WHERE league_id=$1 AND roster_id=$2 AND status='ready' ORDER BY granted_at LIMIT 1 FOR UPDATE SKIP LOCKED",[p.league.id,p.rosterId]);testCrate=test.rows[0]||null;if(testCrate)winKey=`test:${testCrate.id}`;}
      if(!winKey)throw Object.assign(new Error('This franchise has no unopened Victory Crates.'),{status:409});const item=rewardItem(items),owned=await client.query('SELECT 1 FROM efl_cosmetic_inventory WHERE league_id=$1 AND roster_id=$2 AND cosmetic_id=$3',[p.league.id,p.rosterId,item.id]);
