export default async function handler(req,res){
  ...
  try{
    ...
    const sourceWeek=Math.max(1,Number(state.week||1));
    const week=seasonType==='regular'?sourceWeek:1;
    ...
    let previousRosters=[];
    if(league.previous_league_id){try{previousRosters=await get(`${BASE}/league/${league.previous_league_id}/rosters`)}catch{}}
